  ...
  class Dataset
      ...
      if params['layer'] == "*"
        ...
        if params.has_key? 'layer'
          ...
          if is_or_query and is_and_query
            ...
            layer_ids = CitySDK::get_matching_layer_ids(params['layer'].split(LAYER_OR_SEPARATOR))
          elsif is_and_query
            op = :& # boolean AND operator
            layer_ids = CitySDK::get_matching_layer_ids(params['layer'].split(LAYER_AND_SEPARATOR))
          else # just one layer
            layer_ids = CitySDK::get_matching_layer_ids(params['layer'])
          end
        end
    ...
    def geo_bounds(params)
      ...
      if params.has_key? "within"
        ...
        if container
          ...
          CitySDKAPI.do_abort(422,"Containing node not found: #{params[:within]}")
        end
      end
    ...
    def nodedata(params)
      ...
      params.each_pair do |param, values|
        ...
        if match != nil
          layer = match[:layer]
          layer_id = CitySDK::get_matching_layer_ids(layer)[0]
          layer_ids << layer_id
          key = match[:key]

          if (op == :& and values and values.include?(LAYER_OR_SEPARATOR)) or
            (op == :| and values and values.include?(LAYER_AND_SEPARATOR))
            CitySDKAPI.do_abort(422,"'data_op=or' can only be used in combination with 'layer::key=a|b|c', 'data_op=and' can only be used in combination with 'layer::key=a,b,c'. Default is 'and'.")
          end
